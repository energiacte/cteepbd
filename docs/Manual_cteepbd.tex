...
{
morekeywords={EAMBIENTE,BIOCARBURANTE,BIOMASA,BIOMASADENSIFICADA,CARBON,ELECTRICIDAD,GASNATURAL,GASOLEO,GLP,RED1,RED2,SOLAR},
morekeywords={[2]PRODUCCION,CONSUMO,AUX,SALIDA,RED,INSITU,A,B,COGEN,SUMINISTRO,A_RED,A_NEPB,ACS,CAL,REF,VEN,HU,DHU,ILU,NDEF,NEPB,GEN},
sensitive=false,
...
\item \texttt{CTE\_DEMANDA\_ACS\_PCT\_BIOMASA}: porcentaje de la demanda de ACS que se cubre con el consumo del vector \texttt{BIOMASA}. Puede ser necesario para el cálculo de la fracción renovable de la demanda de ACS cuando se genere ACS empleando sistemas que consumen biomasa de distinto tipo y/o vectores que no son \texttt{EAMBIENTE}, \texttt{RED1}, \texttt{RED2} o \texttt{SOLAR};
...
\begin{lstlisting}
1, PRODUCCION, INSITU, ELECTRICIDAD, 34.21, 41.94, 64.94, 73.88, 88.44, 88.64, 91.04, 76.15, 52.84, 39.26, 27.43, 26.26 # PV 5m2 (5kWp)
...
\item \texttt{SOLAR} - Energía térmica procedente de captadores solares
\end{itemize}
\end{multicols}
...
\item \texttt{produced\_by\_source} - la energía producida, por origen y para cada intervalo;
\item \texttt{produced\_by\_source\_an} - la energía producida, por origen y en total anual;
\item \texttt{produced\_used\_EPus} - la energía producida y usada en usos EPB, para cada intervalo;
\item \texttt{produced\_used\_EPus\_an} - energía anual producida y usada en usos EPB;
\item \texttt{produced\_used\_EPus\_by\_source} - la energía producida y usada en usos EPB, por origen y para cada intervalo;
\item \texttt{produced\_used\_EPus\_by\_source\_an} - energía anual producida y usada en usos EPB, por origen;
...
\item \texttt{exported\_by\_source} - la energía exportada, por origen y para cada intervalo;
\item \texttt{exported\_by\_source\_an} - la energía exportada, por origen, en total anual;
...
\texttt{balance\_cr.ELECTRICIDAD.produced\_used\_EPus\_an}
\end{myquote}
