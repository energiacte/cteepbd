...
\textbf{\copyright ~ 2018-2025 Ministerio de Fomento}\\
\textbf{\phantom{\copyright ~ 2018-2025 }Instituto de Ciencias de la Construcción Eduardo Torroja (IETcc-CSIC)}\\[5pt]
\textbf{Autores}:\\[5pt]
...
Este argumento indica la ruta del archivo que define los componentes energéticos sobre los que se realiza el cálculo de la eficiencia energética. Tiene el formato definido en el apartado \nameref{sec:formatocomponentes} e incluye metadatos que pueden definir también algunos parámetros de cálculo, aunque con menor prioridad que las opciones definidas explícitamente a través de la interfaz del programa. El valor '-' indica la lectura de los datos desde la entrada estándar.
...
%
%Este argumento indica que el balance tendrá en consideración únicamente los componentes asociados al servicio de ACS y unos factores de paso definidos para el perímetro próximo (\textit{nearby}).
...
\textbf{\texttt{-{}-json\_output}}

Este argumento sustituye la salida de texto convencional a la salida estándar por el resultado de la evaluación en formato JSON. Esta opción es incompatible con las opciones \texttt{-v} y \texttt{-{}-licencia}.
...
\textbf{\texttt{-{}-xml\_version}}

Este argumento indica la versión del formato XML a utilizar para la salida de datos. Puede adoptar los valores \texttt{2.1} o \texttt{3.0}.
...
\item \texttt{CTE\_DEMANDA\_ACS\_PCT\_BIOMASA}: valor numérico que indica el porcentaje de la demanda de ACS cubierto por biomasa;
...
En el caso de que se quiera obtener el fragmento XML de acuerdo a la versión 2.1 es necesario indicar la opción \texttt{-{}-xml\_version 2.1}:

\begin{Verbatim}[fontsize=\small]
    $ cteepbd -c componentes.cteepbd -l PENINSULA --xml fragmentocert21.xml --xml_version 2.1
\end{Verbatim}
%$
...
\label{sec:anexocambios}
\setcounter{figure}{0} % resetear contador por sección
\setcounter{table}{0} % resetear contador por sección

Este apéndice detalla los cambios realizados entre versiones.

\subsection{v.0.25.0 (junio 2026)}

\begin{itemize}
\item  Nueva opción \texttt{-{}-xml\_version} para indicar si se quiere que el XML generado en el archivo sea con el formato de la versión 2.1 o 3.0
\end{itemize}

\subsection{v.0.24.3 (mayo 2026)}

\begin{itemize}
\item  Corrección de panic para generar un error al no localizar algunos factores de paso
\end{itemize}

\subsection{v.0.24.2 (mayo 2026)}

\begin{itemize}
\item  Corrección de uso de factores de paso explícitamente indicados en el caso de vectores insitu (ELECTRICIDAD, MEDIOAMBIENTE) y no siempre ren 1.0, nren 0.0, co2 0.0
\end{itemize}

\subsection{v.0.24.0 (julio 2025)}

\begin{itemize}
\item Posibilidad de leer el archivo de componentes usando la entrada estándar con \texttt{-c '-'}.
\item Posibilidad de sacar resultados JSON por la salida estándar con opción \texttt{-J, -{}-json\_output}.
\item Adaptación de la salida del fragmento XML a la versión v3.0 del Certificado de Eficiencia Energética del Edificio en Formato XML.
\item Muestra en la versión, además del número, también el commit id y fecha con el que se ha compilado el programa.
\item Compilación para arquitectura x86\_64 (64-bit).
\end{itemize}

\subsection{v.0.23.0 (octubre 2020)}

\begin{itemize}
\item Versión inicial.
\end{itemize}

\subsection{v.0.22.0 (septiembre 2020)}

\begin{itemize}
\item  Corrección del fragmento XML (\<CO2> -{}-> \<co2>)
\end{itemize}

\subsection{0.21.0 (junio 2020)}

\begin{itemize}
\item  El cálculo del porcentaje renovable de la demanda de ACS en el perímetro próximo ahora admite más casos, con las siguientes restricciones:
  \begin{itemize}
  \item  Si se produce ACS con biomasa, solo se puede combinar con otra producción in situ
  \item  No admite producción de ACS con electricidad cogenerada
  \end{itemize}
\item  Se corrige el cálculo del porcentaje renovable de la demanda de ACS en el perímetro próximo, que en algunos casos se reportaba superior al 100%.
\end{itemize}

\subsection{0.20.0 (junio 2020)}

\begin{itemize}
\item  El cálculo de la parte renovable de la demanda de ACS en perímetro próximo es ahora una operación infalible (siempre devuelve un resultado). En el caso de que el indicador no se pueda calcular, se añade en `balance.misc` una clave \texttt{error\_acs} donde se dan los detalles, evitando la salida abrupta del programa.
\end{itemize}

\subsection{0.19.0 (junio 2020)}

\begin{itemize}
\item  Posibilidad de excluir en el cálculo de la demanda renovable de ACS de los componentes marcados con \texttt{CTEEPBD\_EXCLUYE\_AUX\_ACS} en el comentario. Esto permite excluir los consumos auxiliares eléctricos, que no contribuyen a la demanda.
\item  Posiblidad de indicar la demanda anual de ACS en los metadatos con \texttt{CTE\_ACS\_DEMANDA\_ANUAL}
\end{itemize}

\subsection{0.18.0 (junio 2020)}

\begin{itemize}
\item  Documentación de los códigos de salida de la aplicación (OK y errores)
\end{itemize}

\begin{itemize}
\item  Detalla error al recibir demanda anual de ACS nula para el cálculo de la parte renovable de la demanda (emite error)
\end{itemize}

\subsection{0.17.0 (mayo 2020)}

\begin{itemize}
\item  Corrección del reparto de la producción eléctrica por servicios en presencia de consumos No EPB
\end{itemize}
