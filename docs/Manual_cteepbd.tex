...
{
morekeywords={ELECTRICIDAD,BIOCARBURANTE,BIOMASA,CARBON,FUELOIL,GASNATURAL,GASOLEO,GLP,MEDIOAMBIENTE,BIOMASADENSIFICADA,RED1,RED2},
morekeywords={[2]PRODUCCION,CONSUMO,EPB,NEPB,RED,INSITU,A,B,COGENERACION,input,to_grid,to_nEPB,ACS,CAL,REF,VEN,HU,DHU,ILU,NDEF},
sensitive=false,
...
\textbf{\copyright ~ 2018 Ministerio de Fomento}\\
\textbf{\phantom{\copyright ~ 2018 }Instituto de Ciencias de la Construcción Eduardo Torroja (IETcc-CSIC)}\\[5pt]
\textbf{Autores}:\\[5pt]
\-\qquad Grupo de Energética Edificatoria y Sostenibilidad de la Unidad de Calidad en la Construcción:\\
...
La salida incluye los componentes energéticos (\texttt{components}), los factores de paso (\texttt{wfactors}), el área de referencia (\texttt{arearef}), el factor de exportación (\texttt{k\_exp}), el balance energético para cada intervalo de cálculo y por vector energético (\texttt{balance\_cr\_i}), el balance global (\texttt{balance}), y el balance global repercutido por superficie (\texttt{balance\_m2}).
...
Los balances globales (\texttt{balance}, \texttt{balance\_m2}) incluyen la parte renovable y no renovable de la energía primaria en el paso A (\texttt{A}), en el paso B (\texttt{B}), la energía primaria suministrada (\texttt{we\_del}), la energía primaria exportada en el paso A (\texttt{we\_exp\_A}) y la energía exportada en el paso B (\texttt{we\_exp}).
...
Opcionalmente, puede solicitarse la salida en formato XML o JSON, siendo este última la más completa de todas ellas y la más estable.
